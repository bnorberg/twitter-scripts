...
### by full path, name, and extention to your desired output file (ie, /place/on/filesystem/tweets.csv)
	def create_csv
		check_directory_exits(ARGV.last)
		tweets_file = ARGV[1]
		...
		CSV.open(tweets_file, 'ab') do |csv|
			begin
			  	file = CSV.read(tweets_file,:encoding => "iso-8859-1",:col_sep => ",")
				  	if file.none?
				    	csv << ["id_str", "from_user", "from_user_screen_name", "from_user_id_str", "text", "retweets", "created_at", "time", "geocoordinates", "user_lang", "in_reply_to_id_str", "in_reply_to_screen_name", "source", "user_followers_count", "user_friends_count", "likes", "user_location", "status_url", "hashtags", "mentions", "urls", "media"]
				  	end
	  				...
	  				tweets.each do |tweet|
						puts tweet
						puts "------------"
	  					csv << [tweet['id'], tweet['user']['name'], tweet['user']['screen_name'], tweet['user']['id'], tweet['full_text'], tweet['retweet_count'], tweet['created_at'], change_dateformat(tweet['created_at']), get_coordinates(tweet['coordinates']), tweet['user']['lang'], tweet['in_reply_to_user_id'], tweet['in_reply_to_screen_name'], Sanitize.fragment(tweet['source']), tweet['user']['followers_count'], tweet['user']['friends_count'], tweet['favorite_count'], tweet['user']['location'], "http://twitter.com#{tweet['screen_name']}/statuses/#{tweet['id']}", get_hashtags(tweet['entities']['hashtags']), get_mentions(tweet['entities']['user_mentions']), get_urls(tweet['entities']['urls']), get_media(tweet)]
	  				end	
	  		rescue Exception => e
 				puts "Error #{e}"
 				next
	...
	def get_media(tweet)
		if !tweet['entities']['media'].nil?
			@file_directory = "#{ARGV.last}/#{tweet['id']}"
					...
					if rm['media_url_https'].include?("video")
						if !rm['video_info'].nil?
							if rm['video_info']['variants'][0]['url'].include?(".mp4")
								@file_url = rm['video_info']['variants'][0]['url'].split("/?")[0]
							else
								@file_url = rm['video_info']['variants'][1]['url'].split("/?")[0]
							end		
							@media_array << @file_url
						end
