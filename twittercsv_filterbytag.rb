		  				...
		  				begin
						  	file = CSV.read(tweets_file,:encoding => "iso-8859-1",:col_sep => ",")
				  				...
				  				@tags.each do |tag|
				  					if !tweet['text'].nil?
				  						if tag_present == "no"
						  					if tweet['text'].include?(tag.strip)
						  						tag_present = "yes"
						  						puts "#{tweet['id']}--#{tag}--#{tag_present}"
...
sfm_files = TagSearch.new(ARGV[0])
sfm_files.readfiles
